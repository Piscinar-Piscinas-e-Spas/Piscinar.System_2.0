                ...
                <div class="card border-primary-subtle">
                    ...
                    <div class="card-body">
                        <div class="row g-3 align-items-end">
                            <div class="col-md-7">
                                <label class="form-label">Nome do cliente (seleção + digitação)</label>
                                <input type="hidden" id="clienteId" value="">
                                <input type="text" class="form-control mb-2" id="clienteNome" placeholder="Digite para filtrar a seleção de clientes">
                                <select class="form-select" id="clienteSelect" size="4">
                                ...
                                <small class="text-muted">Digite para filtrar. Clique no cliente da seleção ou pressione Enter para preencher os dados automaticamente.</small>
                            </div>
                            <div class="col-md-3">
...
<script>
    ...
    function preencherClienteSelecionado(option) {
        ...
        clienteNomeInput.value = option.dataset.nome || option.textContent || '';
        clienteIdInput.value = option.value || '';
        document.getElementById('clienteTelefone').value = option.dataset.telefone || '';
        document.getElementById('clienteCpfCnpj').value = option.dataset.cpfcnpj || '';
        return true;
    }
    ...
    clienteNomeInput.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') {
            event.preventDefault();
            const selecionado = clienteSelect.selectedOptions[0]
                || Array.from(clienteSelect.options).find((opt) => !opt.hidden);
            preencherClienteSelecionado(selecionado);
        }
    });
    ...
    clienteSelect.addEventListener('click', () => {
        preencherClienteSelecionado(clienteSelect.selectedOptions[0]);
    });
