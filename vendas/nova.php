...
<script>
    ...
    function preencherClientePorNome() {
        const nomeDigitado = normalizarTexto(clienteNomeInput.value);
        const opcoes = Array.from(document.querySelectorAll('#clientesNomesList option'));
        const opcao = opcoes.find((opt) => normalizarTexto(opt.value) === nomeDigitado);
        ...
        clienteNomeInput.value = opcao.value;
        clienteIdInput.value = opcao.dataset.id || '';
        document.getElementById('clienteTelefone').value = opcao.dataset.telefone || '';
        document.getElementById('clienteCpfCnpj').value = opcao.dataset.cpfcnpj || '';
        return true;
    }

    clienteNomeInput.addEventListener('input', preencherClientePorNome);
    clienteNomeInput.addEventListener('change', preencherClientePorNome);
    clienteNomeInput.addEventListener('keydown', (event) => {
        if (event.key === 'Enter') {
            preencherClientePorNome();
        }
    });
